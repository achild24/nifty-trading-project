...
export const Trade = () => {
  ...
  const shortQuantity = ownedPosition?.quantity > 0 ? Math.max(0, numericQuantity - ownedPosition.quantity) : numericQuantity;
  const shortMarginRequired = stock ? stock.price * Math.max(0, shortQuantity) : 0;
  ...
  const handleTrade = async (type) => {
    ...
    if (type === 'SELL' && stock.price * Math.max(0, ownedPosition?.quantity > 0 ? tradeQuantity - ownedPosition.quantity : tradeQuantity) > availableMargin) {
      const message = 'Insufficient margin for this short sell';
      setError(message);
      showToast('error', message);
      return;
    }
