...
export const Trade = () => {
  ...
  const availableMargin = Math.max(0, availableBalance - marginUsed);
  ...
  const shortQuantity = ownedPosition?.quantity > 0
    ? Math.max(0, numericQuantity - ownedPosition.quantity)
    : numericQuantity;
  const shortMarginRequired = stock && Number.isFinite(shortQuantity) ? stock.price * shortQuantity : 0;
  const hasSufficientMargin = shortMarginRequired <= availableMargin;
  ...
  return (
    <div className="rounded-2xl bg-white p-6 shadow-sm border border-slate-200">
      ...
      <aside ref={orderPanelRef} className={`absolute right-0 top-0 z-40 flex h-[520px] w-full max-w-[300px] flex-col rounded-xl border border-slate-200 bg-slate-50 p-5 shadow-lg transition-transform duration-300 ease-out max-sm:fixed max-sm:bottom-0 max-sm:left-0 max-sm:right-0 max-sm:top-auto max-sm:h-auto max-sm:max-w-none max-sm:rounded-b-none max-sm:rounded-t-2xl max-sm:shadow-2xl max-sm:transition-transform ${stock ? 'translate-x-0 max-sm:translate-x-0 max-sm:translate-y-0' : 'translate-x-[calc(100%+1rem)] max-sm:translate-x-0 max-sm:translate-y-full'}`} aria-hidden={!stock}>
        ...
        {stock ? <><p className="mt-2 text-sm text-slate-500">Selected stock</p><h4 className="mt-1 text-2xl font-bold text-slate-800">{stock.symbol}</h4><p className="mt-1 text-lg font-semibold text-indigo-600">₹{stock.price.toLocaleString()}</p><label className="mt-5 block text-sm font-medium text-slate-700" htmlFor="trade-quantity">Quantity</label><input id="trade-quantity" type="number" min="1" step="1" value={quantity} onChange={(e) => setQuantity(e.target.value)} onBlur={() => setQuantity(Math.max(1, parseInt(quantity, 10) || 1))} className="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2" /><div className="mt-4 space-y-2 rounded-xl border border-slate-200 bg-white p-3 text-sm"><div className="flex items-center justify-between"><span className="text-slate-500">Estimated amount</span><strong className="text-slate-800">₹{orderAmount.toLocaleString()}</strong></div><div className="flex items-center justify-between"><span className="text-slate-500">Available balance</span><strong className="text-slate-800">₹{availableBalance.toLocaleString()}</strong></div><div className="flex items-center justify-between"><span className="text-slate-500">Available margin</span><strong className="text-slate-800">₹{availableMargin.toLocaleString()}</strong></div><div className="flex items-center justify-between border-t border-slate-100 pt-2"><span className="text-slate-500">After Buy</span><strong className={hasSufficientBalance ? 'text-slate-800' : 'text-red-600'}>₹{(availableBalance - orderAmount).toLocaleString()}</strong></div><div className="flex items-center justify-between"><span className="text-slate-500">After Sell</span><strong className="text-emerald-700">₹{(availableBalance + orderAmount).toLocaleString()}</strong></div><p className={`font-semibold ${hasSufficientBalance ? 'text-emerald-600' : 'text-red-600'}`}>{hasSufficientBalance ? 'Sufficient balance to Buy' : 'Insufficient balance to Buy'}</p>{shortMarginRequired > 0 && <p className={`text-xs font-semibold ${hasSufficientMargin ? 'text-emerald-600' : 'text-red-600'}`}>{hasSufficientMargin ? `Short margin available: ₹${shortMarginRequired.toLocaleString()}` : `Insufficient short margin: ₹${shortMarginRequired.toLocaleString()} required`}</p>}{ownedQuantity > 0 && <p className="text-xs text-slate-500">Current holding: {ownedQuantity} share{ownedQuantity === 1 ? '' : 's'}</p>}</div><div className="mt-5 grid grid-cols-2 gap-2"><button type="button" disabled={!hasSufficientBalance || orderAmount <= 0} onClick={() => handleTrade('BUY')} className="w-full rounded-lg bg-emerald-600 px-4 py-2.5 font-semibold text-white hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-50">Buy</button><button type="button" disabled={orderAmount <= 0 || !hasSufficientMargin} onClick={() => handleTrade('SELL')} className="w-full rounded-lg bg-red-600 px-4 py-2.5 font-semibold text-white hover:bg-red-700 disabled:cursor-not-allowed disabled:opacity-50">Sell</button></div></> : <p className="mt-3 text-sm leading-6 text-slate-500">Click a stock in the market table to open its buy and sell options.</p>}
      </aside>
      </div>
